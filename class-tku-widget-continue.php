...
class TKU_Widget_Continue extends Widget_Base {
    public function get_name() { return 'tku_continue'; }
    public function get_title() { return 'Ügy folytatása (Temetkezés)'; }
    ...
    protected function register_controls() {
        ...
        $this->add_control('title', [
            ...
            'default' => 'Ügyintézés folytatása',
        ]);

        $this->add_control('label_back', [
            'label' => '„Vissza” gomb',
            'type' => Controls_Manager::TEXT,
            'default' => 'Vissza',
        ]);

        $this->add_control('label_save', [
            'label' => '„Mentés” gomb',
            'type' => Controls_Manager::TEXT,
            'default' => 'Mentés',
        ]);
        ...
        $this->add_control('label_finalize', [
            'label' => '„Véglegesítés” gomb',
            'type' => Controls_Manager::TEXT,
            'default' => 'Véglegesítés',
        ]);
        ...
        $this->add_control('subtitle', [
            ...
            'rows' => 3,
            'default' => 'Kérjük lépésről lépésre töltsd ki az adatokat. A mentés után folytathatod később is.',
    ...
    protected function render() {
        ...
        if (!empty($s['label_back'])) $atts['label_back'] = $s['label_back'];
        if (!empty($s['label_save'])) $atts['label_save'] = $s['label_save'];
        if (!empty($s['label_next'])) $atts['label_next'] = $s['label_next'];
        if (!empty($s['label_finalize'])) $atts['label_finalize'] = $s['label_finalize'];
